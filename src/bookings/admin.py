...
def cancel_bookings(modeladmin, request, queryset):
    updated = queryset.filter(status=Booking.STATUS_ACTIVE).update(
...
class BookingAdmin(admin.ModelAdmin):
    ...
    def status_colored(self, obj):
        colors = {Booking.STATUS_ACTIVE: "green", Booking.STATUS_CANCELLED: "red"}
        return format_html(
            '<b style="color:{}">{}</b>', colors.get(obj.status, "black"), obj.status
        )
